...
void FSM()
{
    ...
    switch (state) {
        ...
        case DECODE:
            ...
            // find next state depending on opcode
            if (IR_meta->type == R_TYPE) state = EXEC;
            else if (opcode == EOP) state = EXIT_STATE;
            //else if(opcode == 8 ) state = I_TYPE_EXEC;
            else if (opcode == 42) state = EXEC;   
...
void execute()
{
    ...
    int jpm_offset = IR_meta->jmp_offset;
    jpm_offset = (jpm_offset) << 2 ;  // get 28 bits of jump address
    int mask = 4026531840;
    int getPC = curr_pipe_regs->pc & mask ; // get 4 upper bits of pc 
    int address = getPC | jpm_offset; //pc+4 in 4 other bits and 28 lower bits we calculated before
    ...
    switch (control->ALUOp) {
        case 0:
            ...
            break;
        case 1:
         //   printf("BEQ: %d - %d == %d\n", alu_opA, alu_opB, alu_opA-alu_opB);
             next_pipe_regs->ALUOut = alu_opA - alu_opB;
             break;    
        case 2:
       //     printf("R-TYPE instruction, function code: %u\n", IR_meta->function);
            if (IR_meta->function == ADD)
                next_pipe_regs->ALUOut = alu_opA + alu_opB;
            else if(IR_meta->function==SLT){
                if(alu_opA - alu_opB < 0)  next_pipe_regs->ALUOut = 1;
                else next_pipe_regs->ALUOut = 0;
            }    
            else{
                printf("mf69\n");
                assert(false);
    ...
    switch (control->PCSource) {
        ...
        case 2 :
           //calculate address in case of a jump
           next_pipe_regs->pc = address; // set pc counter to jump adress
           break;
...
void write_back()
{
    if (arch_state.control.RegWrite) {
        ...
        else   write_data =  arch_state.curr_pipe_regs.ALUOut;
